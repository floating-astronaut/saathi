...
def test_the_actual_leak_that_happened_is_caught():
    ...
    leaked = 'me/accounts -> {"access_token": "EAAEXAMPLEfaketoken0123456789abcdefGHIJKL"}'
    out = np.redact(leaked)
    assert "EAAEXAMPLEfa" not in out and np.REDACTED in out
...
@pytest.mark.parametrize("secret", [
    "shpca_EXAMPLEfaketoken0123456789abcdef",
