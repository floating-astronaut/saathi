...
| Database | `saathi` on local **Postgres 18.4**, role `saathi`, port 5432 — the only cluster on the box. Matches the original box's major, so custom-format dumps move between them in both directions. Upgraded from 16.14 on 2026-07-30 via PGDG + `pg_upgradecluster`; the 16 cluster was dropped the same day, so **rollback is now restore-from-backup, not a port change**. Nearest copies: `/var/backups/saathi/pre-pg18/` (pre-upgrade dump + globals) and `s3://saathi-dev-artifacts-635860424621/backups/postgres/`. |
