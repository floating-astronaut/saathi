...
| Database | `saathi` on local **Postgres 18.4**, role `saathi`, port 5432. Matches the original box's major, so custom-format dumps move between them in both directions. Upgraded from 16.14 on 2026-07-30 via PGDG + `pg_upgradecluster`; the old 16 cluster is retained **stopped on port 5433** as a rollback and can be released with `pg_dropcluster 16 main`. |
