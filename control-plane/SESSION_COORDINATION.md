...
|-------|-----|---------|------|----------------------|--------|
| Claude | runtime `ip-172-31-41-224` | 2026-07-30 | AGENT-1 | `saathi/agent/tools/handlers.py`, `saathi/agent/prompt.py`, `saathi/agent/tools/specs.py`, `tests/test_lookup.py`, `CHANGELOG.md`, `control-plane/*` | **in progress** — increment 1: weather→web deterministic fallback + prompt/spec hardening so the agent doesn't give up. Capability confirmed present (web search answers live). 633 tests. Remains: tool-use eval set. Branch `agent/tool-reliability`. |
