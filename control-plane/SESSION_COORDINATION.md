...
|-------|-----|---------|------|----------------------|--------|
| Claude | runtime `ip-172-31-41-224` | 2026-07-30 | SAFE-LANG-1 | `saathi/safety/classifier.py`, `tests/test_safety.py`, `docs/DECISIONS.md`, `docs/PROD_READINESS.md`, `CHANGELOG.md`, `control-plane/*` | **ended** — native-script gu/ml patterns added across all safety families; verified live (16 fire, benign don't). Closes the D-AF gap. 665 tests. Branch `agent/safe-lang-gu-ml` → PR + deploy. |
| Claude | runtime `ip-172-31-41-224` | 2026-07-30 | AGENT-1 | `saathi/agent/tools/handlers.py`, `saathi/agent/prompt.py`, `saathi/agent/tools/specs.py`, `saathi/eval/agent.py`+`agent_cases.py`, `tests/test_lookup.py`+`test_eval_agent.py`, `evals/README.md`, `CHANGELOG.md`, `control-plane/*` | **ended** — inc1: weather→web fallback + prompt/spec hardening. inc2: tool-use eval that RUNS live (fake DB + dry-run handler) — **100% answered well** across 13 cases. AGENT-1 CLOSED. Branches `agent/tool-reliability`, `agent/tool-use-eval`. |
| Claude | runtime `ip-172-31-41-224` | 2026-07-30 | VOICE-2 | `saathi/onboarding.py`, `tests/test_onboarding.py`, `docs/ARCHITECTURE.md`, `CHANGELOG.md`, `docs/ENGINEERING_SUPERVISOR.md`, `control-plane/*` | **ended** — onboarding voiced for voice users (messages-log detection, additive, boundary intact). Opened AGENT-1 (tool-use reliability, Hermes-informed). 632 tests. Branch `agent/voice-onboarding` → PR + deploy. |
| Claude | runtime `ip-172-31-41-224` | 2026-07-30 | LOOKUP-1 | `saathi/lookup/weather.py`, `tests/test_lookup.py`, `CHANGELOG.md`, `docs/ENGINEERING_SUPERVISOR.md`, `control-plane/*` | **ended** — weather now honors an explicitly-named city over the stored home city; phrases + multi-word cities geocode. 628 tests. Branch `agent/weather-explicit-city` → PR + deploy. |
