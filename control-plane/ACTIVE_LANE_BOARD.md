...
### AGENT-1 — tool-use reliability: reach for tools, don't give up   [CLOSED]
Owner: Claude (runtime box, branches agent/tool-reliability + agent/tool-use-eval)        Opened: 2026-07-30 · Closed: 2026-07-30
...
Notes: opened 2026-07-30 from operator feedback ("an agent that can't answer the
  ...
  **Increment 1 landed 2026-07-30 (branch agent/tool-reliability, deployed):**
  ...
  bare place". 1 test. **Boundary reaffirmed:** this is reliable answering/acting,
  NOT code execution or unbounded actions — capability-by-absence stays (no money/
  OTP/account tools).
  **Increment 2 landed 2026-07-30 (branch agent/tool-use-eval, CLOSES the lane):**
  `saathi/eval/agent.py` — a tool-use/QA eval that RUNS against the live model
  (fake DB + dry-run tool handler: real look_up search, mutating tools stubbed, no
  side effects). 13 committed cases across weather/fact/web/direct/action. First
  live run: **100% answered well, 100% right-tool, 0% give-up**. Caught + fixed a
  scorer false-positive (a correct diabetes answer flagged as give-up via a bare
...
### CAPI-1 — Click-to-WhatsApp conversion attribution   [CLOSED]
...
Notes: Goal clarified = Click-to-WhatsApp (CTWA) ad attribution. Docs-first increment done (docs/CAPI_GATEWAY.md), grounded in Meta docs (verified 2026-07-30): capture `ctwa_clid` from the inbound `referral` (already in the payload — pipeline never reads it), send a manual CAPI `LeadSubmitted` to `graph.facebook.com/v21.0/{DATASET_ID}/events` with action_source=business_messaging on onboarding completion. Model B (self-reported events) chosen over Model A (Automatic Events API, where Meta runs NLP over elders' threads) — privacy call. The Cloud Run Gateway + bucket are a web-pixel path this flow does not use; recommend teardown. Token + WABA id already in the secret.
CLOSED 2026-07-30: MET. Operator confirmed Model B and supplied dataset 2038444060213473. Built: migration 016 (ctwa_clid/ctwa_captured_at on users), saathi/capi.py (capture write-once + report LeadSubmitted), call sites in pipeline.py (after resolve) and onboarding.py (at completion), config SAATHI_CAPI_DATASET_ID/TEST_EVENT_CODE. Boundary D-AD. 587 tests (10 new) assert no-content/no-PII, write-once, never-raises. Live probe accepted every field of the exact payload and rejected only a synthetic ctwa_clid ("Messaging Event Invalid Ctwa Clid"). Gateway teardown left as a follow-up. Deployed via PR #36.
...
### RUNTIME-MIGRATION-2 — Phase 2: systemd units + cloudflared + live cutover   [CLOSED]
...
Acceptance: everything Saathi depends on lives in the mcc-org account `635860424621` except a deliberately-scoped Bedrock credential; the app authenticates as the instance role for all non-inference AWS work; inference no longer rides an expiring SSO session; the old box serves nothing and is retired; live evidence for every claim; tests green.
Write-back: CHANGELOG.md, docs/PROD_READINESS.md, docs/RUNBOOK.md, docs/ENGINEERING_SUPERVISOR.md, control-plane/ACTIVE_LANE_BOARD.md, control-plane/SESSION_COORDINATION.md
Notes: MET. Delivered across `01aca03` (estate move: S3 + secrets + SNS/alarms + least-privilege IAM), `c98a98e` (DB migration, 8 users/262 msgs/58 turns, ownership fixed), `d4e0f41` (Postgres 16→18.4 to match, recovery single-headed again), `69bd817` (drop 16 cluster + empty DB), `265aedb` (inference onto a Bedrock-only IAM user, off the borrowed SSO profile). Old box then fully retired: services disabled, instance terminated, root volume + snapshot deleted, old buckets swept, old secrets scheduled for deletion. MeshPilot account now holds only the `saathi-bedrock-invoke` user + dormant alerting. Remaining: MIGRATION-BEDROCK-1 (model access on the new account is NOT_AUTHORIZED — the one reason inference still uses a foreign-account key; see PR-BEDROCK-KEY) and MIGRATION-PG-VERSION-1 is now Resolved.
